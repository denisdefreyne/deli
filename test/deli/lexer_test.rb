...
class TestDeliLexer < Minitest::Test
  def test_numbers
    ...
    assert_token(:MINUS,  '-',   '-',   tokens.shift)
  ...
  def test_binary_operators
    ...
    assert_token(:PLUS,     '+', '+', tokens.shift)
    assert_token(:MINUS,    '-', '-', tokens.shift)
    assert_token(:ASTERISK, '*', '*', tokens.shift)
    assert_token(:SLASH,    '/', '/', tokens.shift)
    assert_token(:EOF,      '',  nil, tokens.shift)
    assert_nil(tokens.shift)
  end
  ...
  def test_pairs
    ...
    assert_token(:LT,       '<', '<', tokens.shift)
    assert_token(:GT,       '>', '>', tokens.shift)
  ...
  def test_relational_operators
    ...
    assert_token(:EQ,      '=',  '=',  tokens.shift)
    assert_token(:EQ_EQ,   '==', '==', tokens.shift)
    assert_token(:BANG_EQ, '!=', '!=', tokens.shift)
    assert_token(:LT,      '<',  '<',  tokens.shift)
    assert_token(:LTE,     '<=', '<=', tokens.shift)
    assert_token(:GT,      '>',  '>',  tokens.shift)
    assert_token(:GTE,     '>=', '>=', tokens.shift)
    assert_token(:EOF,     '',   nil,  tokens.shift)
    assert_nil(tokens.shift)
  end
